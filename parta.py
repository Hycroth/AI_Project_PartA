...
class Gameboard:
    ...
    def update_board(self, oldboard, i, j, k, m):
        ...
        if newboard.is_elim(k, m+1):         # Square on right
            newboard.board[k][m+1] = EMPTY
        if newboard.is_elim(i, j):           # Piece itself 
            newboard.board[i][j] = EMPTY
        return newboard   
    
    # Returns true if piece (i,j) exists and is eliminated
    def is_elim(self, i, j):
        ...
        if ((i-1) >= TOP_BORDER and (i+1) <= BOTTOM_BORDER):
            ...
            if ((top_piece == enemy or top_piece == CORNER) and 
                (bot_piece == enemy or bot_piece == CORNER)):
                return True
        ...
        if ((j-1) >= LEFT_BORDER and (j+1) <= RIGHT_BORDER):
            ...
            if ((left_piece == enemy or left_piece == CORNER) and 
                (right_piece == enemy or right_piece == CORNER)):
                return True
            
        return False
    
    # Returns updated gameboard object if move is possible and None otherwise
...
class Node:
    ...
    # Compares position of white pieces between current node and parent. 
    # Returns the move as a list [start.i,start.j,end.i,end.j]
    def find_move(self):
        # No parent so this is the root
        if self.parent == None:
            return None
        # Whitepiece1 was moved
        elif ((self.wp1.i != self.parent.wp1.i) or 
              (self.wp1.j != self.parent.wp1.j)):
            return [self.parent.wp1.i, self.parent.wp1.j, 
                    self.wp1.i, self.wp1.j]
        
        # Whitepiece2 was moved
        elif ((self.wp2.i != self.parent.wp2.i) or 
              (self.wp2.j != self.parent.wp2.j)):
            return [self.parent.wp2.i, self.parent.wp2.j, 
                    self.wp2.i, self.wp2.j]
...
class Tree:
    ...
    # Returns a list of all leaf nodes by recurrsion
    def find_leaves(self, node):
        leaf_nodes = []
        # Node has no children (base case)
        if not node.children:
            leaf_nodes.append(node)
        # Traverse down children of each node
        else:
    ...
    # Expands the tree by depth of 1
    def expand(self):        
        leaves = self.find_leaves(self.root)
        
        # For each leaf node, attempt to create a child for each possible move.
        # Child is not created if move results in either whitepiece being 
        # eliminated
        for node in leaves:
            oldboard = node.board.board
            wp1 = node.wp1
            wp2 = node.wp2
            bp = node.bp
            ...
            if (newboard != None and newboard.board[wp1.i-1][wp1.j] != EMPTY):
                node.add_child(Node(bp, Piece(wp1.i-1, wp1.j, WHITE), 
                                    node.wp2,newboard))
            # Jump up?
            newboard = node.board.move(oldboard, wp1.i, wp1.j, UP, True)
            if (newboard != None and newboard.board[wp1.i-2][wp1.j] != EMPTY):
                node.add_child(Node(bp, Piece(wp1.i-2, wp1.j, WHITE), 
                                    node.wp2, newboard))
            ...
            if (newboard != None and newboard.board[wp1.i][wp1.j-1] != EMPTY):
                node.add_child(Node(bp, Piece(wp1.i, wp1.j-1, WHITE), 
                                    node.wp2, newboard))
            # Jump left?
            newboard = node.board.move(oldboard, wp1.i, wp1.j, LEFT, True)
            if (newboard != None and newboard.board[wp1.i][wp1.j-2] != EMPTY):
                node.add_child(Node(bp, Piece(wp1.i, wp1.j-2, WHITE), 
                                    node.wp2, newboard))
            ...
            if (newboard != None and newboard.board[wp2.i-1][wp2.j] != EMPTY):
                node.add_child(Node(bp, wp1, Piece(wp2.i-1, wp2.j, WHITE), 
                                    newboard))
            # Jump up?
            newboard = node.board.move(oldboard, wp2.i, wp2.j, UP, True)
            if (newboard != None and newboard.board[wp2.i-2][wp2.j] != EMPTY):
                node.add_child(Node(bp, wp1, Piece(wp2.i-2,wp2.j, WHITE), 
                                    newboard))
            ...
            if (newboard != None and newboard.board[wp2.i][wp2.j-1] != EMPTY):
                node.add_child(Node(bp, wp1, Piece(wp2.i, wp2.j-1, WHITE), 
                                    newboard))
            # Jump left?
            newboard = node.board.move(oldboard, wp2.i, wp2.j, LEFT, True)
            if (newboard != None and newboard.board[wp2.i][wp2.j-2] != EMPTY):
                node.add_child(Node(bp, wp1, Piece(wp2.i, wp2.j-2, WHITE), 
                                    newboard))
...
# Iterative deepening search where we expand the tree each time the depth
# limit is increased
def IDS(tree, gameboard):
    ...
    while(True):
        ...
        if result != None:
            return result
        depth += 1
        # Expand the tree by depth 1 continue searching
        tree.expand()
        
# Returns coordinates [x1, y1, x2, y2] of the two white pieces closest 
# to (i,j)
...
if command == "Moves":
    ...
    while(1):
            ...
            for j in range(BOARD_WIDTH):
                if gameboard[i][j] == player_icon:
                    ...
                    if (i != TOP_BORDER):
                        ...
                        if (i != TOP_BORDER+1 and (gameboard[i-1][j] == WHITE 
                                            or gameboard[i-1][j] == BLACK)):
                            if (gameboard[i-2][j] == EMPTY):
                                tally += 1
                    ...
                    if (i != BOTTOM_BORDER):
                        ...
                        if (i != BOTTOM_BORDER-1 and (gameboard[i+1][j] == WHITE 
                                            or gameboard[i+1][j] == BLACK)):
                            if (gameboard[i+2][j] == EMPTY):
                                tally += 1
                    ...
                    if (j != LEFT_BORDER):
                        ...
                        if (j != LEFT_BORDER+1 and (gameboard[i][j-1] == WHITE 
                                            or gameboard[i][j-1] == BLACK)):
                            if (gameboard[i][j-2] == EMPTY):
                                tally += 1
                    ...
                    if (j != RIGHT_BORDER):
                        ...
                        if (j != RIGHT_BORDER-1 and (gameboard[i][j+1] == WHITE 
                                            or gameboard[i][j+1] == BLACK)):
                            if (gameboard[i][j+2] == EMPTY):
                                tally += 1
        ...
        # Swaps to the second player after the first iteration, 
        # or breaks the loop otherwise
        if (player_icon == WHITE):
            player_icon = BLACK
            enemy_icon = WHITE
        else:
            break
    ...
    for i in range(BOARD_LENGTH):
        for j in range(BOARD_WIDTH):
            ...
            if thegameboard.board[i][j] == BLACK:
                ...
                # Find the final move node for the sequence of moves that 
                # eliminates the black piece
